...
class AIService 
{
    ...
    {
        // Credentials come from config (env-backed) only. No hardcoded secrets.
        $endpoint     = (string) config('ai.azure.endpoint');
        $apiKey       = (string) config('ai.azure.api_key');
        $deploymentId = (string) config('ai.azure.deployment_id');
        $apiVersion   = (string) config('ai.azure.api_version');
