...
.dna-page {
    background: linear-gradient(145deg,#fdf4ff 0%,#f5f3ff 25%,#eef2ff 55%,#f0fdf4 100%);
    min-height:100vh; padding:1.5rem;
}
.dna-hero {
    background:linear-gradient(135deg,#7c3aed 0%,#a855f7 30%,#ec4899 65%,#f97316 100%);
    border-radius:1.5rem; padding:1.75rem 2rem;
    box-shadow:0 12px 40px rgba(124,58,237,.32);
...
.btn-dna-analyze {
    ...
    background:#fff; color:#7c3aed; border:none; cursor:pointer;
    box-shadow:0 3px 12px rgba(0,0,0,.15); transition:all .2s;
}
.btn-dna-analyze:hover { transform:translateY(-2px); box-shadow:0 6px 22px rgba(0,0,0,.2); }
...
.dna-stat {
    ...
    box-shadow:0 4px 20px rgba(0,0,0,.1); color:#fff; animation:dnaSlideIn .4s ease both;
    transition:transform .25s, box-shadow .25s;
}
.dna-stat:hover { transform:translateY(-5px) scale(1.02); box-shadow:0 14px 40px rgba(0,0,0,.18); }
.dna-stat::before { content:''; position:absolute; inset:0; background:linear-gradient(135deg,rgba(255,255,255,.16) 0%,transparent 60%); pointer-events:none; }
...
.stat-dna-pink   { background:linear-gradient(135deg,#ec4899,#f43f5e); }
.stat-dna-violet { background:linear-gradient(135deg,#7c3aed,#6366f1); }
.stat-dna-green  { background:linear-gradient(135deg,#10b981,#059669); }
.stat-dna-amber  { background:linear-gradient(135deg,#f59e0b,#d97706); }
...
/* Section cards */
.dna-card {
    background:linear-gradient(135deg,rgba(255,255,255,.88),rgba(245,243,255,.94));
    border:1.5px solid rgba(139,92,246,.14); border-radius:1.25rem;
    box-shadow:0 4px 20px rgba(99,102,241,.1); overflow:hidden;
    animation:dnaSlideIn .5s ease both;
}
.dna-card-header {
    padding:1rem 1.5rem;
    border-bottom:1.5px solid rgba(139,92,246,.1);
...
.dna-card-title { font-size:.9rem; font-weight:800; color:#1a1a2e; letter-spacing:-.01em; }
...
.archetype-chip:hover { transform:translateX(4px); box-shadow:0 3px 12px rgba(99,102,241,.18); }

/* Trait bars */
.trait-row { padding:.6rem 0; border-bottom:1px solid rgba(139,92,246,.07); }
.trait-row:last-child { border-bottom:none; }
.trait-label { font-size:.8rem; font-weight:600; color:#1a1a2e; margin-bottom:.3rem; display:flex; justify-content:space-between; }
.trait-bar-bg { height:7px; background:rgba(139,92,246,.1); border-radius:9999px; overflow:hidden; }
.trait-bar-fill { height:100%; border-radius:9999px; background:linear-gradient(90deg,#6366f1,#a855f7,#ec4899); background-size:200%; animation:dnaShimmer 3s linear infinite; transition:width .9s cubic-bezier(.22,.68,0,1.2); }
...
.value-chip {
    ...
    color:#7c3aed; border:1px solid rgba(124,58,237,.2);
    transition:transform .15s, box-shadow .15s;
}
.value-chip:hover { transform:scale(1.06); box-shadow:0 3px 10px rgba(124,58,237,.2); }

/* Work style / communication rows */
.pattern-row {
    display:flex; align-items:center; gap:.6rem;
    padding:.55rem .75rem; border-radius:.625rem; font-size:.82rem; font-weight:500; color:#374151;
    transition:background .15s;
}
.pattern-row:hover { background:rgba(99,102,241,.06); }
...
.meta-label { font-size:.7rem; font-weight:700; letter-spacing:.05em; text-transform:uppercase; color:#9ca3af; margin-bottom:.3rem; }
.meta-val   { font-size:1.35rem; font-weight:900; color:#1a1a2e; letter-spacing:-.02em; }

/* Empty state */
.dna-empty {
    text-align:center; padding:5rem 2rem;
    background:linear-gradient(135deg,rgba(255,255,255,.9),rgba(245,243,255,.95));
    border:1.5px solid rgba(139,92,246,.14); border-radius:1.5rem;
    ...
    <div id="dna-content-area">
        ...
        <div class="grid grid-cols-1 lg:grid-cols-2 gap-5">
            <div class="dna-card" style="animation-delay:.25s">
                <div class="dna-card-header">
                    <div class="dna-card-icon" style="background:linear-gradient(135deg,#ec4899,#f43f5e)">
            ...
            <div class="dna-card" style="animation-delay:.3s">
                <div class="dna-card-header">
                    <div class="dna-card-icon" style="background:linear-gradient(135deg,#6366f1,#818cf8)">
                ...
                <div class="dna-card-body space-y-2" id="archetypes-container">
                    <div style="text-align:center;padding:2rem;color:#c4b5fd;font-size:.84rem">Loading archetypes…</div>
                </div>
            </div>
        </div>
        ...
        <div class="grid grid-cols-1 lg:grid-cols-2 gap-5 mt-5">
            <div class="dna-card" style="animation-delay:.35s">
                <div class="dna-card-header">
                    <div class="dna-card-icon" style="background:linear-gradient(135deg,#10b981,#059669)">
                ...
                <div class="dna-card-body" id="success-traits-container">
                    <div style="text-align:center;padding:2rem;color:#c4b5fd;font-size:.84rem">Loading traits…</div>
                </div>
            </div>

            <div class="dna-card" style="animation-delay:.4s">
                <div class="dna-card-header">
                    <div class="dna-card-icon" style="background:linear-gradient(135deg,#a855f7,#7c3aed)">
                ...
                <div class="dna-card-body">
                    <div class="flex flex-wrap gap-2" id="core-values-container">
                        <div style="color:#c4b5fd;font-size:.84rem">Loading values…</div>
                    </div>
                </div>
            </div>
        </div>
        ...
        <div class="grid grid-cols-1 lg:grid-cols-2 gap-5 mt-5">
            <div class="dna-card" style="animation-delay:.45s">
                <div class="dna-card-header">
                    <div class="dna-card-icon" style="background:linear-gradient(135deg,#6366f1,#4f46e5)">
                ...
                <div class="dna-card-body space-y-1" id="work-style-container">
                    <div style="color:#c4b5fd;font-size:.84rem;text-align:center;padding:1rem">Loading…</div>
                </div>
            </div>

            <div class="dna-card" style="animation-delay:.5s">
                <div class="dna-card-header">
                    <div class="dna-card-icon" style="background:linear-gradient(135deg,#06b6d4,#0891b2)">
        ...
        <div class="dna-card mt-5" style="animation-delay:.55s">
            <div class="dna-card-header">
                <div class="dna-card-icon" style="background:linear-gradient(135deg,#64748b,#94a3b8)">
            ...
            <div class="dna-card-body">
                <div class="grid grid-cols-1 sm:grid-cols-3 gap-4 mb-4">
                    <div class="meta-stat" style="background:linear-gradient(135deg,#eef2ff,#e0e7ff)">
                        <div class="meta-label">Last Analyzed</div>
                        <div class="meta-val" id="last-analyzed" style="font-size:1rem">--</div>
                    </div>
                    <div class="meta-stat" style="background:linear-gradient(135deg,#f0fdf4,#bbf7d0)">
                        <div class="meta-label">Employees Analyzed</div>
                        <div class="meta-val" id="employees-analyzed">--</div>
                    </div>
                    <div class="meta-stat" style="background:linear-gradient(135deg,#fdf4ff,#e9d5ff)">
                        <div class="meta-label">Hires Analyzed</div>
                        <div class="meta-val" id="hires-analyzed">--</div>
                    </div>
                </div>
                <div style="padding:1rem 1.25rem;border-radius:.875rem;background:linear-gradient(135deg,rgba(99,102,241,.07),rgba(168,85,247,.05));border:1px solid rgba(99,102,241,.12)">
                    <div style="font-size:.7rem;font-weight:800;letter-spacing:.08em;text-transform:uppercase;color:#a5b4fc;margin-bottom:.5rem">AI Analysis Summary</div>
                    <p style="font-size:.84rem;color:#374151;line-height:1.6" id="ai-summary">Loading analysis summary…</p>
                </div>
            </div>
        </div>
...
function renderDNAProfile(data) {
    ...
    renderList('work-style-container', profile.work_style_preferences ?? [], '#6366f1');
    renderList('communication-container', profile.communication_patterns ?? [], '#06b6d4');
...
function renderRadarChart(dna) {
    ...
    culturalChart = new Chart(ctx, {
        ...
        data: { labels, datasets: [{ label:'Cultural DNA', data:scores,
            backgroundColor:'rgba(168,85,247,.18)', borderColor:'rgba(168,85,247,.9)', borderWidth:2,
            pointBackgroundColor:'rgba(168,85,247,1)', pointBorderColor:'#fff',
            pointHoverBackgroundColor:'#fff', pointHoverBorderColor:'rgba(168,85,247,1)' }]},
        options: { scales:{r:{beginAtZero:true,max:100,ticks:{stepSize:20},grid:{color:'rgba(139,92,246,.1)'},pointLabels:{font:{size:10,weight:'600'},color:'#6366f1'}}},
...
function renderArchetypes(archetypes) {
    ...
    el.innerHTML = archetypes.length
        ...
        : '<p style="color:#c4b5fd;font-size:.84rem;text-align:center;padding:1rem">No archetypes identified</p>';
}
...
function renderTraits(traits) {
    ...
    el.innerHTML = items.length
        ? items.map((t,i) => {
            ...
            return `<div class="trait-row"><div class="trait-label"><span>${i+1}. ${label}</span>${score ? `<span style="color:#10b981;font-weight:800">${score}</span>` : ''}</div>${score ? `<div class="trait-bar-bg"><div class="trait-bar-fill" style="width:${score}%"></div></div>` : ''}</div>`;
        }).join('')
        : '<p style="color:#c4b5fd;font-size:.84rem;text-align:center;padding:1rem">No traits identified</p>';
}
...
function renderValues(values) {
    ...
    el.innerHTML = values.length
        ...
        : '<p style="color:#c4b5fd;font-size:.84rem">No core values defined</p>';
}
...
function renderList(id, items, dotColor) {
    ...
    el.innerHTML = items.length
        ...
        : `<p style="color:#c4b5fd;font-size:.84rem;text-align:center;padding:1rem">No data</p>`;
}
    ...
    area.innerHTML = `
        <div class="dna-empty">
            <div class="dna-empty-icon">
                <svg style="width:2.25rem;height:2.25rem;color:#a78bfa" fill="none" viewBox="0 0 24 24" stroke="currentColor" stroke-width="1.5">
                    <path stroke-linecap="round" stroke-linejoin="round" d="M4 7v10c0 2.21 3.582 4 8 4s8-1.79 8-4V7M4 7c0 2.21 3.582 4 8 4s8-1.79 8-4M4 7c0-2.21 3.582-4 8-4s8 1.79 8 4m0 5c0 2.21-3.582 4-8 4s-8-1.79-8-4"/>
                </svg>
            </div>
            <h3 style="font-size:1.2rem;font-weight:900;color:#1a1a2e;margin-bottom:.5rem">No DNA Profile Found</h3>
            <p style="color:#9ca3af;font-size:.84rem;margin-bottom:1.5rem">${msg ?? 'DNA profile not found. Please run DNA analysis first.'}</p>
            <button onclick="analyzeDNA(event)" style="display:inline-flex;align-items:center;gap:.5rem;padding:.75rem 1.75rem;border-radius:.875rem;font-size:.875rem;font-weight:700;background:linear-gradient(135deg,#7c3aed,#a855f7);color:#fff;border:none;cursor:pointer;box-shadow:0 4px 18px rgba(124,58,237,.35);transition:all .2s" onmouseover="this.style.transform='translateY(-2px)'" onmouseout="this.style.transform='translateY(0)'">
                <svg style="width:15px;height:15px" fill="none" viewBox="0 0 24 24" stroke="currentColor" stroke-width="2.5"><path stroke-linecap="round" stroke-linejoin="round" d="M9.75 17L9 20l-1 1h8l-1-1-.75-3M3 13h18M5 17h14a2 2 0 002-2V5a2 2 0 00-2-2H5a2 2 0 00-2 2v10a2 2 0 002 2z"/></svg>
                <span id="analyze-btn-text">Run DNA Analysis</span>
            </button>
        </div>`;
