...
.credit-card-hover:hover { transform: translateY(-3px); box-shadow: 0 8px 24px rgba(99,102,241,.15); }
...
@endpush
    ...
    <div class="grid grid-cols-1 sm:grid-cols-3 gap-5 mb-8">
        {{-- Credits Remaining --}}
        <div class="rounded-2xl p-5 credit-card-hover" style="background:linear-gradient(135deg,#4f46e5,#7c3aed,#a855f7);box-shadow:0 4px 20px rgba(99,102,241,.3)">
        ...
        {{-- Used This Month --}}
        <div class="rounded-2xl p-5 credit-card-hover" style="background:linear-gradient(135deg,#fdf4ff,#ede9fe);border:1.5px solid #c084fc">
            <p class="text-xs font-semibold uppercase tracking-wider" style="color:#7e22ce">Used This Month</p>
            <p class="text-4xl font-bold mt-2" style="color:#4a044e">{{ $thisMonth }}</p>
            <p class="text-xs mt-1" style="color:#9333ea">credits consumed</p>
        </div>

        {{-- All Time --}}
        <div class="rounded-2xl p-5 credit-card-hover" style="background:linear-gradient(135deg,#eef2ff,#e0e7ff);border:1.5px solid #818cf8">
            <p class="text-xs font-semibold uppercase tracking-wider" style="color:#3730a3">All Time Total</p>
            <p class="text-4xl font-bold mt-2" style="color:#1e1b4b">{{ $totalUsed }}</p>
            <p class="text-xs mt-1" style="color:#4338ca">credits ever used</p>
        </div>
    </div>
    ...
    <div class="grid grid-cols-1 lg:grid-cols-3 gap-6">
        ...
        <div class="lg:col-span-2">
            <div class="rounded-2xl overflow-hidden" style="border:1.5px solid #a5b4fc;box-shadow:0 4px 20px rgba(99,102,241,.1)">
                ...
                @if($logs->isEmpty())
                    <div class="px-6 py-16 text-center">
                        <div class="inline-flex items-center justify-center w-16 h-16 rounded-2xl mb-4" style="background:linear-gradient(135deg,#ede9fe,#ddd6fe)">
                ...
                @else
                    <div class="divide-y" style="divide-color:#e0e7ff">
                        @foreach($logs as $log)
                            ...
                            <div class="px-6 py-4 flex items-center justify-between hover:bg-indigo-50/40 transition-colors">
                                ...
                                <div class="flex-shrink-0 ml-4 text-right">
                                    <span class="inline-flex items-center gap-1 px-3 py-1 rounded-full text-sm font-bold" style="background:#fef3c7;color:#92400e">
                    ...
                    {{-- Pagination --}}
                    @if($logs->hasPages())
                        <div class="px-6 py-4" style="border-top:1px solid #e0e7ff">
                            {{ $logs->links() }}
                        </div>
                    @endif
                @endif
        ...
        <div class="space-y-5">
            <div class="rounded-2xl overflow-hidden" style="border:1.5px solid #c084fc;box-shadow:0 4px 16px rgba(168,85,247,.1)">
                <div class="px-5 py-4" style="background:linear-gradient(135deg,#7e22ce,#a855f7)">
                ...
                <div class="p-5 space-y-4" style="background:linear-gradient(180deg,#fdf4ff,#f5f3ff)">
                    @forelse($byAction as $item)
                        @php
                            $color = \App\Models\AICreditLog::$actionColors[$item->action] ?? ['bg'=>'#f3f4f6','text'=>'#374151'];
            ...
            {{-- Where to use AI --}}
            <div class="rounded-2xl p-5" style="background:linear-gradient(135deg,#eef2ff,#e0e7ff);border:1.5px solid #818cf8">
                <h3 class="text-sm font-bold mb-3" style="color:#3730a3">Where to Use AI Credits</h3>
                <div class="space-y-2.5">
                    @foreach([
                        ...
                        <a href="{{ $feat['url'] }}" class="flex items-center justify-between p-2.5 rounded-xl hover:bg-white/60 transition-colors group">
                            ...
                            <span class="text-xs font-bold px-2 py-0.5 rounded-full" style="background:#fef3c7;color:#92400e">{{ $feat['cost'] }} cr</span>
                        </a>
                    @endforeach
                </div>
