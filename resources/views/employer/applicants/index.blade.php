...
.app-back-btn {
    ...
    border:1.5px solid rgba(20, 71, 186,.2); color:#1B57C4;
    box-shadow: none;
    text-decoration:none; transition:all .2s;
}
.app-back-btn:hover { transform:translateX(-3px); box-shadow: none; color:#0C2E72; }
...
.btn-export {
    display:inline-flex; align-items:center; gap:.5rem;
    padding:.6rem 1.25rem; border-radius:.875rem; font-size:.8rem; font-weight:700;
    background:#fff; color:#1B57C4; border:none;
    box-shadow: none; text-decoration:none; transition:all .2s; cursor:pointer;
}
.btn-export:hover { transform:translateY(-2px); box-shadow: none; }
...
.app-input {
    width:100%; border-radius:.75rem; border:1.5px solid rgba(20, 71, 186,.2);
    background:rgba(255,255,255,.9); padding:.6rem 1rem;
    font-size:.84rem; color:#0C0C0C; outline:none; transition:border-color .2s, box-shadow .2s;
    font-family:'Plus Jakarta Sans',sans-serif;
}
.app-input:focus { border-color:#2D6CDF; box-shadow: none; }
.app-input::placeholder { color:#BFCFEE; }
.btn-apply {
...
.btn-apply:hover { transform:translateY(-2px); box-shadow: none; }
.btn-clear {
    padding:.62rem 1rem; border-radius:.75rem; font-size:.84rem; font-weight:600;
    background:rgba(20, 71, 186,.1); color:#1B57C4; border:1.5px solid rgba(20, 71, 186,.2);
    text-decoration:none; transition:all .2s; white-space:nowrap;
}
.btn-clear:hover { background:rgba(20, 71, 186,.18); }

/* -- STATUS TABS -- */
.app-tabs-card {
    background:rgba(255,255,255,.88);
    border:1.5px solid rgba(20, 71, 186,.14); border-radius:1.25rem; overflow:hidden;
    box-shadow: none;
}
.app-tab {
    display:inline-flex; align-items:center; gap:.5rem;
    padding:.75rem 1.25rem; font-size:.82rem; font-weight:600;
    text-decoration:none; color:#2D6CDF; transition:all .15s; white-space:nowrap;
    border-bottom:2.5px solid transparent; position:relative;
}
.app-tab:hover { background:rgba(20, 71, 186,.07); color:#0C2E72; }
.app-tab.active {
    color:#2D6CDF; border-bottom-color:#2D6CDF;
    background:rgba(20, 71, 186,.07);
}
.tab-badge {
...
.app-table thead tr {
    background:#EBF2FF;
    border-bottom:1.5px solid rgba(20, 71, 186,.15);
}
.app-table th {
...
.app-table tbody tr {
    border-bottom:1px solid rgba(20, 71, 186,.07);
    transition:background .15s, transform .15s;
}
.app-table tbody tr:last-child { border-bottom:none; }
.app-table tbody tr:hover {
    background:rgba(20, 71, 186,.05);
}
...
.app-cand-name  { font-size:.875rem; font-weight:700; color:#0C0C0C; }
.app-cand-email { font-size:.72rem; color:#A8A8A8; margin-top:.1rem; }
.app-job-title  { font-size:.84rem; font-weight:600; color:#0C0C0C; }
.app-job-loc    { font-size:.72rem; color:#A8A8A8; margin-top:.1rem; }
.app-date-main  { font-size:.82rem; font-weight:600; color:#0C0C0C; }
.app-date-rel   { font-size:.7rem; color:#A8A8A8; }
...
.status-pending     { background:#FFF8EC; color:#E37400; }
.status-reviewing   { background:#EBF2FF; color:#1B57C4; }
.status-shortlisted { background:#EDFAF2; color:#1E8E3E; }
.status-rejected    { background:#FEF2F2; color:#2D6CDF; }
.status-hired       { background:#EBF2FF; color:#1B57C4; }

/* -- VIEW DETAILS BUTTON -- */
.btn-view-detail {
    display:inline-flex; align-items:center; gap:.35rem;
    padding:.4rem .9rem; border-radius:.625rem; font-size:.78rem; font-weight:700;
    background:#EBF2FF; color:#2D6CDF;
    border:1.5px solid rgba(20, 71, 186,.2); text-decoration:none;
    transition:all .2s;
}
.btn-view-detail:hover { background:#2D6CDF; color:#fff; transform:translateY(-1px); box-shadow: none; border-color:transparent; }
...
<div class="space-y-5">
    ...
    <div class="app-tabs-card">
        <div class="flex flex-wrap">
            @php
                $tabs = [
                    ['label'=>'All',         'status'=>null,          'count'=>$statusCounts['all']??0,        'badge'=>'background:#EBF2FF;color:#2D6CDF'],
                    ['label'=>'Pending',     'status'=>'pending',     'count'=>$statusCounts['pending']??0,    'badge'=>'background:#FFF8EC;color:#E37400'],
                    ['label'=>'Reviewing',   'status'=>'reviewing',   'count'=>$statusCounts['reviewing']??0,  'badge'=>'background:#EBF2FF;color:#1B57C4'],
                    ['label'=>'Shortlisted', 'status'=>'shortlisted', 'count'=>$statusCounts['shortlisted']??0,'badge'=>'background:#EDFAF2;color:#1E8E3E'],
                    ['label'=>'Rejected',    'status'=>'rejected',    'count'=>$statusCounts['rejected']??0,   'badge'=>'background:#FEF2F2;color:#2D6CDF'],
                ];
            @endphp
            @foreach($tabs as $tab)
                @php
    ...
    @if(isset($applications) && $applications->isEmpty())
        <div class="app-empty">
            <div style="width:4rem;height:4rem;border-radius:1.25rem;background:#EBF2FF;display:flex;align-items:center;justify-content:center;margin:0 auto 1.25rem;box-shadow: none;">
                <svg class="w-8 h-8" style="color:#BFCFEE" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                    <path stroke-linecap="round" stroke-linejoin="round" stroke-width="1.5" d="M9 12h6m-6 4h6m2 5H7a2 2 0 01-2-2V5a2 2 0 012-2h5.586a1 1 0 01.707.293l5.414 5.414a1 1 0 01.293.707V19a2 2 0 01-2 2z"/>
                </svg>
            </div>
            <h2 style="font-size:1.15rem;font-weight:800;color:#0C0C0C;margin-bottom:.4rem">No Applications Found</h2>
            <p style="font-size:.84rem;color:#A8A8A8;">
                            ...
                            <td style="white-space:nowrap">
                                <div style="position:relative;width:2.5rem;height:2.5rem;">
                                    <svg style="width:2.5rem;height:2.5rem;transform:rotate(-90deg)" viewBox="0 0 40 40">
                                        <circle cx="20" cy="20" r="16" stroke="#EBF2FF" stroke-width="3" fill="none"/>
                                        <circle cx="20" cy="20" r="16"
                                            stroke="{{ $score >= 80 ? '#1E8E3E' : ($score >= 60 ? '#E37400' : '#2D6CDF') }}"
                                            stroke-width="3" fill="none"
                                            stroke-dasharray="{{ round($score * 1.005) }} 100.5"
                                            stroke-linecap="round"/>
                                    </svg>
                                    <span style="position:absolute;inset:0;display:flex;align-items:center;justify-content:center;font-size:.68rem;font-weight:800;color:#0C0C0C">{{ $score }}</span>
                                </div>
                            </td>
                            {{-- Status --}}
                            <td style="white-space:nowrap">
