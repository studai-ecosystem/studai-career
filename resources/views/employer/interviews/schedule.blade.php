...
@push('styles')
<style>
.sch-page { padding:1.75rem; background:#EBF2FF; min-height:100%; }
.sch-card { background:#fff; border-radius:1.25rem; border:1px solid rgba(20, 71, 186,.1); box-shadow: none; padding:2rem; }
.form-group { margin-bottom:1.25rem; }
.form-group label { display:block; font-size:.78rem; font-weight:700; color:#3D3D3D; margin-bottom:.4rem; }
.form-control { width:100%; padding:.6rem .875rem; border:1.5px solid #E2E2E0; border-radius:.75rem; font-size:.875rem; color:#0C0C0C; transition:border .15s; box-sizing:border-box; }
.form-control:focus { outline:none; border-color:#2D6CDF; box-shadow: none; }
...
.cand-box { background:#EBF2FF; border-radius:1rem; padding:1rem 1.25rem; margin-bottom:1.75rem; display:flex; align-items:center; gap:1rem; }
.cand-av { width:3rem; height:3rem; border-radius:50%; background:#2D6CDF; color:#fff; display:flex; align-items:center; justify-content:center; font-size:1.1rem; font-weight:700; flex-shrink:0; }
</style>
@endpush
...
<div style="max-width:760px;margin:0 auto;">
    ...
    <div style="display:flex;align-items:center;gap:1rem;margin-bottom:1.5rem;">
        <a href="{{ route('employer.interviews.index') }}" style="color:#737373;text-decoration:none;font-size:.85rem;">&#8592; Interview Pipeline</a>
    </div>

    <h1 style="font-size:1.4rem;font-weight:800;color:#0C0C0C;margin:0 0 1.5rem;">
        Phase 2 — Schedule Interview
        @if($existingRound > 0)
        <span style="font-size:.9rem;font-weight:600;color:#2D6CDF;margin-left:.5rem;">(Round {{ $existingRound + 1 }})</span>
        @endif
    </h1>
    ...
    <div class="cand-box">
        ...
        <div>
            <div style="font-weight:700;color:#0C0C0C;">{{ $application->user->name ?? 'Guest' }}</div>
            <div style="font-size:.82rem;color:#2D6CDF;">{{ $application->job->title ?? 'N/A' }} &bull; {{ $application->user->email ?? '' }}</div>
        </div>
    </div>

    <div class="sch-card">
        @if($errors->any())
        <div style="background:#FEF2F2;border:1px solid #FCA5A5;color:#2D6CDF;padding:.875rem;border-radius:.75rem;margin-bottom:1.25rem;font-size:.85rem;">
        ...
        <form method="POST" action="{{ route('employer.interviews.store') }}">
            ...
            <div class="form-group">
                ...
                <div style="font-size:.72rem;color:#A8A8A8;margin-top:.3rem;">Leave blank to auto-generate an in-platform room link</div>
            </div>
            ...
            <div style="display:flex;gap:1rem;align-items:center;padding-top:.5rem;">
                ...
                <a href="{{ route('employer.interviews.index') }}" style="color:#737373;font-size:.85rem;text-decoration:none;">Cancel</a>
            </div>
        </form>
    </div>
